...
export class Queue<QueueType> {
    ...
    private front: number = 0 //front of the queue
    private tail: number = 0 //end of the queue
    ...
    enQueue = (newItem:QueueType):void => {
        if(this.tail === this.length || (this.front-1) === this.tail) throw new Error('Queue length exceeded')
